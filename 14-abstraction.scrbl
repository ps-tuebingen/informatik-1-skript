...
@margin-note{Dieser Teil des Skripts basiert auf [HTDP/2e] Part III}
...
von Konstanten. Diese Möglichkeit haben wir bereits ausführlich im Abschnitt @secref{dry} besprochen.
...
Häufig benötigt man Konstanten, Funktionen oder Strukturen nur @italic{lokal}, innerhalb einer Funktion. Für diesen Fall
...
Mit Hilfe einer Sequenz lokaler Konstantendeklarationen können wir also die Zwischenergebnisse repräsentieren und mit nur 3 Multiplikationen
...
Im Allgemeinen verwendet man lokale Konstanten aus zwei Gründen: 1) Um Redundanz zu vermeiden, oder 2) um Zwischenergebnissen einen Namen zu geben.
...
Mit Hilfe lokaler Konstanten können wir den Zwischenergebnissen für die neue x- und y-Koordinate einen Namen geben
...
Als fortgeschrittenes Beispiel für die Verwendung lokaler Konstanten können wir die Verallgemeinerung des Potenzierungsbeispiels auf beliebige Exponenten betrachten.
...
Es kann vorkommen, dass es mehrere Definitionen eines Namens (Konstanten, Funktion, Strukturkonstruktoren/-selektoren) gibt, die einen
...
Wird die @racket[delta-x] Konstante während der Auswertung von @racket[(f 5)] etwa zu @racket[10000]
...
Funktionsdefinitionen "desugaren" zu Konstantendefinitionen:
...
Die @racket[lambda]-Ausdrücke machen also sehr deutlich, dass Funktionen "ganz normale" Werte sind, an die wir Konstanten binden
können.  Wenn wir also bei obiger Definition von @racket[f] einen Funktionsaufruf @racket[(f e-1 ... e-n)] haben, so ist
das @racket[f] an der ersten Position kein Funktionsname, sondern ein Konstantenname, der zu einem λ-Ausdruck ausgewertet wird.
...
Globale und lokale Funktions- und Konstantendefinitionen, Abstrakte Typsignaturen, Abstrakte Datendefinitionen.
